...
public class EncodingDetector {
    ...
    private List<String> encodeFilters = new ArrayList<String>();
    private List<String> fileExts = new ArrayList<String>();
    /**
     * 严格模式加载整个文件内容进行比对，效率较低，非严格模式检查前10000个字符
     */
    private boolean strictDetect = false;
	 ...
	 * @return 文件的编码
	 */
	public String parseFileEncoding(File f){
		BytesEncodingDetect s = new BytesEncodingDetect(); 
		String fileCode = BytesEncodingDetect.javaname[s.detectEncoding(f, strictDetect ? 0 : 10000)];
		return fileCode;
	}
     ...
     * @return
     */
    public void printFileEncoding(File f, List<String> filterEncode) {
        if (f.isHidden()) {
            return ;
        }
        if (f.isDirectory()) {
            ...
            for (int i = 0; i < list.length; i++) {
                printFileEncoding(list[i], filterEncode);
            }
        } else {
            try {
                if (this.fileExts != null && !this.fileExts.isEmpty()) {
                    Optional<String> findAny = this.fileExts.parallelStream().filter(ext->f.getName().endsWith(ext)).findAny();
                    if (!findAny.isPresent()) {
                        return ;
                    }
                }
                String encode=parseFileEncoding(f);
                if (filterEncode.isEmpty() || filterEncode.contains(encode)) {
                    System.out.println(String.format("%s, encoding=%s",
                        f.getAbsolutePath(), encode));
                }
            } catch (Exception e) {
                System.out.println(String.format("%s, encoding parse error! %s",
     ...
     * @param buff
     */
    public void printByteEncoding(byte[] buff) {
        BytesEncodingDetect s = new BytesEncodingDetect(); 
        String fileCode = BytesEncodingDetect.javaname[s.detectEncoding(buff)];
        System.out.println(String.format("encoding=%s", fileCode));
    }
    ...
    public static void main(String[] args) {
        ...
        byte[] buff = null;
        List<String> filterEncode = new ArrayList<String>();
        ...
        while (i >= 0) {
            ...
            if (param.indexOf("-filter=") == 0) {
                // 过滤文件类型
                String filterStr = param.substring("-filter=".length());
                filterEncode = Arrays.asList(filterStr.split(","));
            } else if (param.indexOf("-file=") == 0) {
                // 文件名
                fileName = param.substring("-file=".length());
            } else if (param.indexOf("-byte=") == 0) {
                String sbyte = param.substring("-byte=".length());
                String[] split = sbyte.split(",");
                buff = new byte[split.length]; 
                for (int j = 0; j < split.length; j++) {
                    buff[j] = Byte.valueOf(split[j]);
                }
            }else if (param.indexOf("-exts=") == 0) {
                exts = Arrays.asList(param.substring("-exts=".length()).split(","));
            }
        ...
        if (fileName == null && buff == null) {
            System.out.println("【此工具用于检查文件字符集】");
            System.out.println("调用方式1：-file=文件名 [-filter=扩展名1,扩展名2...] [-exts=文件扩展名]");
            System.out.println("调用方式2：-byte=字节1,字节2... ");
            System.out.println("  -file 输入有效的文件或目录名称，将对指定文件或指定目录下所有文件进行扫描 ");
            System.out.println("  -filter 指定扫描检测的扩展名，可以输入多个");
            System.out.println("  -byte 直接输入字节数组内容检测字符集");
            System.out.println("  -exts 扩展名，逗号分隔，如java,css,js");
            return;
        }
        
        EncodingDetector detector = new EncodingDetector();
        detector.setEncodeFilters(filterEncode);
        exts = exts.stream().map(s->s.startsWith(".")?s : "."+s).collect(Collectors.toList());
        detector.setFileExts(exts);
        
//        if (!params.isEmpty()) {
//            fileName = params.get(0);
//        } else {
//            System.out.println("【此工具用于检查文件字符集】");
//            fileName = readOneInput("输入一个文件名或路径:");
...
//        }
        ...
        if (fileName != null) {
            ...
            System.out.println("开始检查文件字符集...");
            detector.printFileEncoding(f, filterEncode);
            System.out.println("检查文件字符集完毕！");
            //        readFile(file,encode);
        } else if (buff != null) {
            detector.printByteEncoding(buff);
        }
    }

    /**
     * @return the encodeFilters
     */
    public List<String> getEncodeFilters() {
        return encodeFilters;
    }

    /**
     * @param encodeFilters the encodeFilters to set
     */
    public void setEncodeFilters(List<String> encodeFilters) {
        this.encodeFilters = encodeFilters;
    }
    ...
    /**
     * @param fileExts the fileExts to set
     */
    ...
    /**
     * @return the strictDetect
     */
    public boolean isStrictDetect() {
        return strictDetect;
    }

    /**
     * @param strictDetect the strictDetect to set
     */
    public void setStrictDetect(boolean strictDetect) {
        this.strictDetect = strictDetect;
    }
...
//    }
     ...
     */
    public int detectEncoding(File testfile, long length) {
      FileInputStream chinesefile;
      byte[] rawtext;
      length = length == 0 ? testfile.length() : Math.min(length, testfile.length());
      rawtext = new byte[(int)length];
      try {
    ...
    int gb2312_probability(byte[] rawtext) {
      ...
      // fengyq added -1. if not, it recognize GBK as GB2312 
